...
class MissionPadDetectionDirectionRequest(BaseModel):
    """ミッションパッド検出方向設定リクエスト"""
...
class TrackingStartRequest(BaseModel):
    ...
    tracking_mode: TrackingMode = Field(TrackingMode.CENTER, description="追跡モード")


class ModelTrainRequest(BaseModel):
    """モデル訓練リクエスト"""
    object_name: str = Field(..., description="学習対象オブジェクト名")
    # images field is handled separately in the router as UploadFile